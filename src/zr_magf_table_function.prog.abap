...
*---------------------------------------------------------------------*
DATA:
  gt_data TYPE STANDARD TABLE OF zcds_magf_m3a2,
...
START-OF-SELECTION.
  ...
  IF p_connid IS NOT INITIAL.
    SELECT *
      FROM zcds_magf_m3a2_parameter( p_connid = @p_connid )
      INTO TABLE @gt_data.
  ELSE.
    SELECT *
        FROM zcds_magf_m3a2
...
*---------------------------------------------------------------------*
  ...
  gr_alv->get_display_settings( )->set_list_header( |Flights - CONNID { p_connid }| ).
  gr_alv->display( ).
